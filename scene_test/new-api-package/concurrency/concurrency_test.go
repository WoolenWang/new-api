...
func (s *ConcurrencyTestSuite) SetupSuite() {
	...
	// 启动测试服务器（包含独立进程的 NewAPI + miniredis）
	var err error
	...
	// 为当前测试进程初始化 Redis 客户端，使 service.CheckAndConsumeSlidingWindow
	// 使用与测试服务器相同的 miniredis 实例。
	if s.server == nil || s.server.MiniRedis == nil {
		s.T().Fatal("Test server MiniRedis is not initialized")
	}

	s.redisClient = redis.NewClient(&redis.Options{
		Addr: s.server.MiniRedis.Addr(),
	})

	ctx := context.Background()
	if _, err := s.redisClient.Ping(ctx).Result(); err != nil {
		s.T().Fatalf("Failed to connect to MiniRedis from test process: %v", err)
	}

	// 绑定到全局 Redis 客户端，供 service 层使用
	common.RDB = s.redisClient
	common.RedisEnabled = true
	// 确保 Lua 脚本 SHA 使用新的 Redis 实例
	service.ResetSlidingWindowScriptCache()

	s.T().Logf("Redis client for sliding window initialized at %s", s.server.MiniRedis.Addr())
}
...
func (s *ConcurrencyTestSuite) SetupTest() {
	...
	// 清空 Redis 中的所有窗口 Key，确保测试相互隔离
	if s.server != nil && s.server.MiniRedis != nil {
		s.server.MiniRedis.FlushAll()
	}

	// 每个用例前重置 Lua 脚本缓存，确保脚本在新 Redis 状态下重新加载
	service.ResetSlidingWindowScriptCache()
}
...
func (s *ConcurrencyTestSuite) TestCR01_LuaScriptAtomicity_ConcurrentDeduction() {
	...
	errors := s.runConcurrent(goroutineCount, func(i int) error {
		// 调用滑动窗口检查（真实 service 层调用）
		ctx := context.Background()
		result, err := service.CheckAndConsumeSlidingWindow(ctx, sub.Id, config, int64(requestQuota))
		if err != nil {
	...
	// 7. 通过 error slice 交叉验证成功请求数量
	successFromErrors := s.countSuccessfulRequests(errors)
	assert.Equal(s.T(), int(finalSuccessCount), successFromErrors,
		"Success count from atomic counter should match success count derived from errors slice")
...
func (s *ConcurrencyTestSuite) TestCR02_WindowCreation_ConcurrentRace() {
	...
	// 确保 Redis 中无窗口（首次请求场景）
	windowHelper := testutil.NewRedisWindowHelper(s.server.MiniRedis)
	assert.NoError(s.T(), windowHelper.DeleteWindow(sub.Id, "hourly"))
	...
	// 2. 验证只创建了1个窗口
	pattern := fmt.Sprintf("subscription:%d:hourly:*", sub.Id)
	windowCount := windowHelper.CountWindowKeys(pattern)
	...
	// 验证 consumed = goroutineCount × requestQuota
...
func (s *ConcurrencyTestSuite) TestCR04_MultiPackage_ConcurrentDeduction() {
	...
	// 创建两个套餐
	pkgA := testutil.CreateTestPackage(s.T(), testutil.PackageTestData{
		Name:         "CR-04-Package-A",
		Priority:     pkgAPriority,
		P2PGroupId:   0,
		Quota:        500000000,
		HourlyLimit:  int64(pkgALimit),
		DurationType: "month",
		Duration:     1,
		Status:       1,
	})
	pkgB := testutil.CreateTestPackage(s.T(), testutil.PackageTestData{
		Name:         "CR-04-Package-B",
		Priority:     pkgBPriority,
		P2PGroupId:   0,
		Quota:        500000000,
		HourlyLimit:  int64(pkgBLimit),
		DurationType: "month",
		Duration:     1,
		Status:       1,
	})
	...
	// 用户订阅两个套餐
	subA := testutil.CreateAndActivateSubscription(s.T(), user.Id, pkgA.Id)
	subB := testutil.CreateAndActivateSubscription(s.T(), user.Id, pkgB.Id)

	// 创建滑动窗口配置
	configA := service.SlidingWindowConfig{
		Period:   "hourly",
		Duration: 3600,
		Limit:    int64(pkgALimit),
		TTL:      4200,
	}
	configB := service.SlidingWindowConfig{
		Period:   "hourly",
		Duration: 3600,
		Limit:    int64(pkgBLimit),
		TTL:      4200,
	}
	...
	errors := s.runConcurrent(goroutineCount, func(i int) error {
		...
		// 1. 尝试套餐 A（高优先级）
		ctx := context.Background()
		resultA, err := service.CheckAndConsumeSlidingWindow(ctx, subA.Id, configA, int64(requestQuota))
		...
		// 2. 套餐 A 超限或调用失败时，尝试套餐 B
		resultB, err := service.CheckAndConsumeSlidingWindow(ctx, subB.Id, configB, int64(requestQuota))
		...
		atomic.AddInt32(&failureCount, 1)
		if err != nil {
			return fmt.Errorf("both packages exceeded or failed: %w", err)
		}
	...
	assert.Equal(s.T(), totalConsumed, totalRedisConsumed,
		"Sum of Redis consumed for both packages should match total consumed")
...
func (s *ConcurrencyTestSuite) TestCR05_SubscriptionActivation_ConcurrentConflict() {
	...
	errors := s.runConcurrent(goroutineCount, func(i int) error {
		// 调用订阅启用接口，模拟 /api/subscriptions/:id/activate 的并发行为。
		// 使用模型层的原子状态转换，保证只有一个 goroutine 能成功从
		// inventory → active，其余返回 "invalid status"。
		now := common.GetTimestamp()
		err := model.ActivateSubscription(sub.Id, now)
		if err != nil {
	...
	// 4. 验证最终订阅状态为active
	finalSub, err := model.GetSubscriptionByIdFromDB(sub.Id)
	assert.NoError(s.T(), err)
	...
	// 5. 验证时间字段正确性：EndTime ≈ StartTime + Duration
	if finalSub.StartTime != nil && finalSub.EndTime != nil {
		actualDuration := *finalSub.EndTime - *finalSub.StartTime
		// 对于 month 类型的套餐，实际天数可能在 28~31 天之间，
		// 这里使用区间断言而非固定 30 天，避免不同月份带来的偏差。
		minDuration := int64(28 * 24 * 3600)
		maxDuration := int64(31 * 24 * 3600)
		assert.GreaterOrEqual(s.T(), actualDuration, minDuration,
			"Duration should be at least 28 days")
		assert.LessOrEqual(s.T(), actualDuration, maxDuration,
			"Duration should be at most 31 days")
	}

	// 6. 通过 error slice 交叉验证：应有 1 个成功、1 个失败
	successFromErrors := s.countSuccessfulRequests(errors)
	assert.Equal(s.T(), int(finalSuccessCount), successFromErrors,
		"Success count from atomic counter should match success count derived from errors slice")
...
func (s *ConcurrencyTestSuite) TestCR06_TotalConsumed_ConcurrentUpdate() {
	...
	errors := s.runConcurrent(goroutineCount, func(i int) error {
		// 调用原子更新函数，模拟 PostConsumeQuota 中的 DB 更新逻辑
		err := model.IncrementSubscriptionConsumed(sub.Id, int64(quotaPerUpdate))
		if err != nil {
			atomic.AddInt32(&failureCount, 1)
			return err
		}
	...
	// - 最终结果：100000（丢失了一次更新）
	//
	// 使用GORM Expr("total_consumed + ?", quota)可避免此问题（零容忍）
