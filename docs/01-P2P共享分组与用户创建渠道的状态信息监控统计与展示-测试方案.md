...
*   `User-A`: 系统分组 `vip` (倍率: 2.0)，作为渠道提供者。
*   `User-B`: 系统分组 `default` (倍率: 1.0)，作为渠道使用者。
*   `User-C`: 系统分组 `svip` (倍率: 0.8)，用于测试分组聚合。

### 3.2 渠道 (Channels)

*   `Ch-A`: Owner: User-A, 模型: `gpt-4`, 系统分组: `vip`。
*   `Ch-B`: Owner: User-B, 模型: `gpt-4`, 系统分组: `default`。
*   `Ch-C`: Owner: User-C, 模型: `gpt-3.5`, 系统分组: `svip`。
*   `Ch-Mock-Baseline`: 一个高质量的基准渠道，用于生成模型基准。
*   `Ch-Mock-Test1`: 模拟正常质量的渠道。
*   `Ch-Mock-Test2`: 模拟降智渠道。

### 3.3 P2P分组 (Groups)

*   `G1-Public`: Owner: User-A, 类型: 共享, 成员: User-A, User-B。
*   `G2-Private`: Owner: User-C, 类型: 私有, 成员: User-C。

### 3.4 监控策略 (Monitor Policies)

*   `Policy-Style`: 监控 `gpt-4` 模型的 `style` 检测，每4小时执行一次。
*   `Policy-Encoding`: 监控 `gpt-3.5` 模型的 `encoding` 检测，每6小时执行一次。

### 3.5 模型基准 (Model Baselines)

*   `Baseline-GPT4-Style`: 使用 `Ch-Mock-Baseline` 生成的 `gpt-4` 风格基准。
*   `Baseline-GPT35-Encoding`: 使用 `Ch-Mock-Baseline` 生成的 `gpt-3.5` 编码基准。

### 3.6 Mock服务 (Mock Servers)
