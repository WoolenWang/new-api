...
为保证整套监控与统计用例在 CI / 本地开发环境中可以在**单次 `go test` ≤ 5 分钟**内完成，同时又不改变业务语义，本测试方案对时间相关行为采用了“配置加速 + 用例自适应等待”的策略：

* 在 `scene_test/new-api-monitoring-stats/*` 中，通过 `cfg.CustomEnv` 统一注入加速配置，例如：  
  `CHANNEL_STATS_FLUSH_INTERVAL_SECONDS=2`、`CHANNEL_STATS_WINDOW_SECONDS=10`、`CHANNEL_STATS_SYNC_INTERVAL_SECONDS=2`，并在部分测试中同步写入当前进程环境变量，供等待逻辑读取。
* 原设计中按“1 分钟 flush、15 分钟窗口”编写的用例（如 CL-02/03/04/05、CON-02 等），已经改为**根据上述环境变量计算等待时间**，而不再固定 `time.Sleep(65 * time.Second)`，从而在加速配置下只等待数秒即可完成同等语义的验证（例如“至少经历多个 flush 周期”）。
* 高并发场景（CON-01/02/03/04 以及并发多渠道场景）中，通过环境变量关闭 `GLOBAL_API_RATE_LIMIT_ENABLE` 与 `CRITICAL_RATE_LIMIT_ENABLE`，避免限流逻辑成为瓶颈，确保测试关注点仅在“原子性、并发安全和统计一致性”。
* 所有场景一律使用 **In-Memory SQLite + Miniredis + Mock LLM Server**，避免真实外部网络和持久化实例带来的不确定耗时；个别用例中遗漏的外部 BaseURL 已补齐指向 Mock 上游，保证测试完全可控、可重复。

上述加速策略保证：**生产环境仍使用分钟级窗口和真实限流策略**，而测试环境在秒级窗口下运行，通过环境变量+自适应等待机制维持与设计文档一致的行为验证。
