...
| **S-03-A**| **粘性渠道超额后自动解绑** | 1. 会话A成功绑定到渠道C1。<br>2. 通过其他请求耗尽C1的`hourly_quota_limit`。<br>3. 使用会话A再次发起请求。 | 1. `session:{...}` HASH键被删除。<br>2. 随后创建了指向新渠道（非C1）的新绑定。 | 1. 请求被重新路由到健康的渠道C2。<br>2. 后续该会话的请求将粘滞在C2。 | **P0** |
| **S-03-B**| **粘性渠道被禁用后自动解绑** | 1. 会话A成功绑定到渠道C1。<br>2. 通过API或DB将渠道C1设置为“已禁用”。<br>3. 使用会话A再次发起请求。 | 1. `session:{...}` HASH键被删除。<br>2. 随后创建了指向新渠道（非C1）的新绑定。 | 1. 请求被重新路由到健康的渠道C2。<br>2. 后续该会话的请求将粘滞在C2。 | **P0** |
...
| **DC-03**| **并发创建会话** | 1. 用户并发上限为1。<br>2. 几乎同时发送两个**不同** session_id 的新会话请求（sid=1, sid=2）。 | `SCARD session:user:{id}` 最终应为1；实现层使用 `session:pending:user:{id}` 作为并发创建占位集合，确保 active+pending 不超过上限。 | 只有一个请求成功创建会话并返回200，另一个因并发竞争失败而返回429。 | P1 |
| **DC-04**| **多 Token 叠加并发限制** | 1. 同一用户下创建两个不同的 API Token（可配置不同 BillingGroupList）。<br>2. 将该用户/分组的并发会话上限设为1。<br>3. 几乎同时使用两个 Token 分别创建不同 `session_id` 的新会话请求。 | 通过 `SCARD session:user:{id}` 及日志确认仅有一个活跃会话；`session:pending:user:{id}` 在请求结束后应为空。 | 恰好一个请求返回200，另一个返回429，验证并发限制按“用户维度”生效，对不同 Token 和计费分组叠加生效。 | P1 |
| **DC-05**| **多计费组 + 分时额度联动** | 1. 为用户创建带 `BillingGroupList=["vip","default"]` 的 Token。<br>2. 在 `vip` 计费组下配置渠道Cvip（设置 `hourly_quota_limit` 并在 Redis 中模拟已用额度达到上限）；在 `default` 组下配置健康渠道Cdef。<br>3. 发送请求。 | `channel_quota:{Cvip}:hourly:{ts}` 的值达到 `hourly_quota_limit`；Cdef 无额度限制。 | 选路阶段因 Cvip 分时额度超限被风控过滤，请求自动从 `BillingGroupList` 中回退到 `default`，最终命中 Cdef 渠道并返回200。 | P2 |
